...
//console.log('FUNDING NEEDED: ', $scope.projects[i].totalCost, $scope.projects[i].totalFunding, $scope.projects[i].funding_needed);
        ...
        $scope.sortTable = function (term) {
		    ...
		    $http.get('/api/v2013/index/select?cb=1418322176016&q=(realm_ss:'+realm+'%20AND%20(schema_s:lwDonor))&rows=500&sort=createdDate_dt+desc,+title_t+asc&start=0&wt=json', { cache: true }).then(function(response) {
...
//>>>>>>> 1f3cd8c4dbc0027ac54964ce3dd4fc26c88f16d6
		            return prev.concat(matches);
						...
						_.each($scope.matches,function(match,key){
									if(match && match.lifewebPrevFunded_ss)
										$scope.matches.splice(key,1);

									if(match && match.d_id==='F6145FBA-419F-C6A8-5E3D-62DD02E36ABD')
										$scope.matches.splice(key,1);

									if(match && match.d_id==='1EE789EC-D3E8-A3A1-8F7A-2F702999C2CB')
											$scope.matches.splice(key,1);

									if(match && match.d_id==='58A7E548-B417-AB35-38A2-AB23E7868F33')
													$scope.matches.splice(key,1);
						});
		    });
		});//	$http.get('/api/v2013/in
